...
def test_sort_chapters_orders_by_number_decimal():
    ...
    assert file_utils.sort_chapters(names) == ["Chapter 1", "Chapter 10.3", "Chapter 10.5"]


def test_split_valid_chapters_keeps_parseable_and_separates_rest():
    valid, invalid = file_utils.split_valid_chapters(
        ["c001", "c010.5", "not_a_chapter", "Chapter 2"]
    )
    assert valid == ["c001", "c010.5", "Chapter 2"]
    assert [name for name, _ in invalid] == ["not_a_chapter"]
    assert "No chapter number" in str(invalid[0][1])


def test_split_valid_volumes_keeps_parseable_and_separates_rest():
    valid, invalid = file_utils.split_valid_volumes(["v01", "random_folder", "Vol.2"])
    assert valid == ["v01", "Vol.2"]
    assert [name for name, _ in invalid] == ["random_folder"]
